...
const ServicesPage = () => {
  ...
  useEffect(() => {
    if (categories.length > 0 && !activeTab) {
      setActiveTab(t('services.allServices'));
    }
  }, [categories, activeTab, t]);
  ...
  const filteredServices = useMemo(() => {
    // If "All Services" tab is selected, return all services
    if (activeTab === t('services.allServices')) {
      return services;
    }
    return getServicesByCategory(activeCategory);
  }, [activeCategory, getServicesByCategory, activeTab, services, t]);
